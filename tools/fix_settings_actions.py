...
def apply(path: Path) -> None:
    ...
    if MAIL_HANDLER not in text:
        if CURRENT_ENSURE in text:
            text = text.replace(CURRENT_ENSURE, MAIL_HANDLER + NEW_ENSURE, 1)
        elif OLD_ENSURE in text:
            text = text.replace(OLD_ENSURE, MAIL_HANDLER + NEW_ENSURE, 1)
        elif NEW_ENSURE in text:
            text = text.replace(NEW_ENSURE, MAIL_HANDLER + NEW_ENSURE, 1)
        else:
            raise SystemExit(f'{path}: could not find Legal/contact Settings actions')
    elif CURRENT_ENSURE in text:
        text = text.replace(CURRENT_ENSURE, NEW_ENSURE, 1)
    ...
    if '<a class=\\\"orbit-settings-icon-action\\\" id=\\\"orbitContactEmail\\\"' in final:
        raise SystemExit(f'{path}: contact email is still an iframe mailto anchor')
    ...
    print(f'Compact Legal + working contact button applied: {path}')
